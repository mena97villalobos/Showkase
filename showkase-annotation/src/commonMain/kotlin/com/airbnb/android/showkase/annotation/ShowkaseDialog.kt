...
/**
 * Used to annotate @Composable functions that represent a `Dialog`, full-screen sheet, or any
 * other composable that would be disruptive to render inline inside the Showkase browser. Instead
 * of composing the function immediately when the entry is visible, the Showkase browser renders a
 * gated toggle button. The annotated composable is only invoked once the user taps the button, and
 * is removed from composition when the button is tapped again.
 *
 * Use this for any composable that takes over the screen, such as
 * `androidx.compose.ui.window.Dialog` previews:
 *
 * @ShowkaseDialog(name = "Confirm dialog", group = "Dialogs")
 * @Composable
 * fun MyDialogPreview() {
 ...
 *
 * @ShowkaseDialog(name = "Name", group = "Group")
 * @Composable
 * fun MyDialogPreview() {
 *     MyDialog("Title")
 * }
 ...
 * @param buttonText Label rendered on the toggle button while the dialog is hidden. If empty, the
 * default localized "Show Dialog" string is used.
 * @param hideButtonText Label rendered on the toggle button while the dialog is visible. If empty,
 * the default localized "Hide Dialog" string is used.
 */
@MustBeDocumented
...
annotation class ShowkaseDialog(
    val name: String = "",
    val group: String = "",
